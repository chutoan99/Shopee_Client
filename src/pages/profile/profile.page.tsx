...
function ProfilePage(): JSX.Element {
	const { refetch } = useGetUserCurrentQuery()
	//? HANDLE ADDRESS
	...
	const onSubmit = () => {
		...
		const newUser: IUser = {
			...
			birthday: payload?.birthday && new Date(payload?.birthday).toISOString().slice(0, 19).replace('T', ' '),
							...
							<div
								...
								<div className='rounded-[50%] bg-[#ccc] mb-[20px]  opacity-0.5 w-[150px] h-[150px] overflow-hidden inline-block'>
									{imagePreview ? (
										...
										<img
											className='w-full f-full'
											src={typeof payload?.avatar === 'string' ? payload.avatar : undefined}
											alt='user'
										/>
									)}
								</div>
