...
import { IPostSimple } from '../interface';
            ...
            >
              ...
              <div className="flex items-baseline justify-between">
                {item?.price_before_discount !== 0 ? (
                  <span className="text-[0.875rem] text-[#666] line-through ml-2.5">{formatPrice(item.price_before_discount)}đ</span>
                ) : (
              ...
              <div className="flex items-center justify-between mt-1.5 mb-0 mx-2.5">
                <span className="text-[rgb(255,66,79)] text-[0.813rem]" onClick={() => changeLike(index)}>
                  <i className={likes[index]}></i>
                </span>
                <div className="text-[0.625rem] origin-right text-[#ccc] ml-auto mr-0 -mt-px mb-0 scale-[0.8]">{generateStart(4)}</div>
              ...
              {item.discount && (
                <div className="absolute text-[rgb(255,216,64,0.94)] bg-current text-[0.625rem] w-10 h-9 text-center right-0 top-0 before:content-[''] before:absolute before:z-0 before:border-t-[0] before:border-x-[20px] before:border-b-[4px] before:border-solid before:border-[transparent_currentColor_transparent_currentColor] before:left-0 before:-bottomo-1">
                  <span className="text-[#ee4d2d] font-semibold text-xs leading-3">{item.discount}</span>
                  <br></br>
                  <span className="text-[#fff] text-[0.813rem] leading-[0.813rem] font-semibold">Giảm</span>
                </div>
              )}
