...
function ModelRattingComponent({
	isShow,
	onCloseModel,
	data
}: RatingModel): JSX.Element {
	...
	const onSubmit = () => {
		...
		if (imageUrls?.length === 0)
			return toast.error('Vui lòng cập nhật hình ảnh')
		const payloadCreateComment = data?.posts?.map(
				...
				<div id='modal'>
					<div>
						<div className='fixed z-[600] inset-0'>
							<div
								className='w-full h-full bg-[rgba(0,0,0,0.4)]'
									...
									<div>
										<div className='max-h-[calc(100vh_-_120px)] w-[45.625rem] shadow-[0_1px_1px_0_rgba(0,0,0,0.05)] max-w-full box-border flex flex-col overflow-hidden pt-[30px] pb-0 px-[30px]  bg-[#fff]'>
											...
											<div
												...
												style={{
													background:
														'linear-gradient(hsla(0, 0%, 100%, 0.9), #fff)'
												}}>
