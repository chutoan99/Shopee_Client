...
import { useGetCartsQuery } from '../../cart/hook'
import { useCreateHistorySearchMutation, useGetHistorySearchQuery } from '../hook'
import { useGetSearchSuggestQuery } from '../../searchSuggest/hook'
import { ISearchHistory } from '../interface/search-history.interface'
import { SuggestListComponent } from '../../searchSuggest/component'
import { HeaderCartComponent } from '../../../components/cart'
        ...
        <div className='h-[82px] flex gap-[5px] items-center px-2 sm-gutter'>
          ...
          <div className='w-full flex justify-center'>
            <div className='w-[90%]'>
                ...
                <div className='flex-1 h-full relative group' id='header_search'>
                  ...
                  <div className='header_list_search  absolute top-[calc(100%_-_-2px)] w-full bg-[#fff] shadow-[0_1px_5px_rgba(189,189,189)] overflow-hidden z-[3] rounded-[3px] left-0'>
                    ...
                    {!isLoadingHistorySearch && (
                      <ul className='mt-1.5'>
                        {dataHistorySearch?.response?.map((item: ISearchHistory, index: number) => (
              ...
              {!isLoadingSearchSuggestion && <SuggestListComponent data={dataSearchSuggestion?.response || []} />}
            </div>
          </div>
          <HeaderCartComponent data={dataCart?.response || [[]]} loading={isLoadingDataCart} totalCart={total_cart} />
        </div>
      </div>
    </>
  )
