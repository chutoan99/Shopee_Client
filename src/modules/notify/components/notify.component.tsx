...
export default function HeaderNotifyComponent({
	data
}: HeaderNotifyModel): JSX.Element {
	return (
		<div
				...
				{data?.map((item: INotify) => {
					return (
						<li
							...
							<NavLink
								to='# '
								className='flex no-underline w-full p-2'>
								<div>
									<img
										src={item.image}
										alt='logoSale'
								...
								<div className='ml-[12px]'>
									...
									<span className='text-xs block text-[#756f6e] leading-4 mt-1'>
										{item.content}
									</span>
								</div>
							</NavLink>
